...
describe('smartSplit', () => {
  ...
  describe('smart mode', () => {
    ...
    it('suggests transfers when deviation exceeds threshold', () => {
      const output = smartSplit(makeInput({ smartMode: true, fairnessThresholdInCents: 100 }));
    ...
    it('uses denomination matching for better distribution', () => {
      ...
      const output = smartSplit(
        makeInput({
          smartMode: true,
          denominations: [makeDenomQty('eur_20', 3), makeDenomQty('eur_10', 4)],
        }),
      );
    ...
    const shares: PersonShare[] = [
      {
        ...
        actualShareInCents: 5100,
        deviationInCents: 100,
      },
      {
        id: 'e2',
        name: 'B',
    ...
    const shares: PersonShare[] = [
      {
        ...
        hoursWorked: 8,
        idealShareInCents: 5000,
        actualShareInCents: 6000,
        deviationInCents: 1000,
      },
      {
        id: 'e2',
        name: 'B',
    ...
    const shares: PersonShare[] = [
      {
        ...
        hoursWorked: 8,
        idealShareInCents: 5000,
        actualShareInCents: 7000,
        deviationInCents: 2000,
      },
      {
        id: 'e2',
        name: 'Small Over',
        ...
        hoursWorked: 8,
        idealShareInCents: 5000,
        actualShareInCents: 5800,
        deviationInCents: 800,
      },
      {
        id: 'e3',
        name: 'Big Under',
...
describe('buildAvailablePool', () => {
  it('converts DenominationQuantity to AvailableDenomination', () => {
    const pool = buildAvailablePool([makeDenomQty('eur_10', 5), makeDenomQty('eur_5', 3)]);
  ...
  it('filters out zero-quantity denominations', () => {
    const pool = buildAvailablePool([makeDenomQty('eur_10', 0), makeDenomQty('eur_5', 3)]);
    expect(pool).toHaveLength(1);
    expect(pool[0]!.denominationId).toBe('eur_5');
  });

  it('filters out unknown denomination IDs', () => {
    const pool = buildAvailablePool([makeDenomQty('unknown_denom', 5)]);
