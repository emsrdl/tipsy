...
describe('exportShiftsCsv', () => {
  ...
  it('generates one row per employee per shift', () => {
    const shift = makeShift({
      distribution: makeSmartDistribution({
        personShares: [
          makePersonShare({ name: 'Anna', role: 'service', hoursWorked: 8, actualShareInCents: 5000 }),
          makePersonShare({ name: 'Bob', role: 'kitchen', hoursWorked: 6, actualShareInCents: 3000 }),
        ],
      }),
    })
  ...
  it('labels roles correctly', () => {
    const shift = makeShift({
      distribution: makeSmartDistribution({
        personShares: [
          makePersonShare({ role: 'kitchen' }),
          makePersonShare({ role: 'service' }),
        ],
      }),
    })
    const csv = exportShiftsCsv([shift])
