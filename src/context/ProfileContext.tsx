...
export function ProfileProvider({ children }: ProfileProviderProps) {
  ...
  const [activeProfileId, setActiveProfileId] = useLocalStorage<string | null>(ACTIVE_PROFILE_KEY, null)
  ...
  const updateProfileStats = useCallback(
    (profileId: string, stats: ProfileStats) => {
      setProfiles((prev) =>
        prev.map((p) => (p.id === profileId ? { ...p, stats } : p))
      )
    },
    [setProfiles]
  )
  ...
  const resetProfileStats = useCallback(
    (profileId: string) => {
      setProfiles((prev) =>
        prev.map((p) =>
          p.id === profileId ? { ...p, stats: { ...EMPTY_STATS } } : p
        )
      )
    },
    [setProfiles]
