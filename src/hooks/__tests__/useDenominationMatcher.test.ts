...
describe('matchDenominations', () => {
  describe('basic assignment', () => {
    ...
    it('uses multiple denominations to reach target', () => {
      const result = matchDenominations({
        ...
        available: [avail('eur_50', 1, 5000), avail('eur_20', 1, 2000)],
      });
    ...
    it('distributes among multiple employees', () => {
      const result = matchDenominations({
        distributions: [dist('e1', 'Anna', 3000), dist('e2', 'Bob', 2000)],
        available: [avail('eur_20', 1, 2000), avail('eur_10', 3, 1000)],
      });
  ...
  describe('edge cases', () => {
    ...
    it('handles single employee getting all cash', () => {
      const result = matchDenominations({
        ...
        available: [avail('eur_50', 1, 5000), avail('eur_20', 2, 2000), avail('eur_10', 1, 1000)],
      });
    ...
    it('reports unallocated denominations', () => {
      ...
      const totalUnallocated = result.unallocated.reduce(
        (s, u) => s + u.available * u.valueInCents,
        0,
      );
  ...
  describe('fairness', () => {
    it('reports perfect fairness when all deviations are zero', () => {
      const result = matchDenominations({
        distributions: [dist('e1', 'Anna', 2000), dist('e2', 'Bob', 3000)],
        available: [avail('eur_20', 1, 2000), avail('eur_10', 1, 1000), avail('eur_20b', 1, 2000)],
      });

      if (result.payouts[0]!.deviationInCents === 0 && result.payouts[1]!.deviationInCents === 0) {
        expect(result.fairness.isPerfect).toBe(true);
        expect(result.fairness.score).toBe(100);
      }
    });
    ...
    it('computes max deviation correctly', () => {
      const result = matchDenominations({
        distributions: [dist('e1', 'Anna', 5000), dist('e2', 'Bob', 5000)],
  ...
  describe('denomination assignment correctness', () => {
    ...
    it('assignment totalCents matches count × value', () => {
      const result = matchDenominations({
        ...
        available: [avail('eur_50', 1, 5000), avail('eur_20', 1, 2000), avail('eur_5', 1, 500)],
      });
    ...
    it('actual amount equals sum of assignments', () => {
      const result = matchDenominations({
        distributions: [dist('e1', 'Anna', 5000), dist('e2', 'Bob', 3000)],
        available: [avail('eur_50', 1, 5000), avail('eur_20', 1, 2000), avail('eur_10', 1, 1000)],
      });
    ...
    it('deviation equals actual minus ideal', () => {
      const result = matchDenominations({
        distributions: [dist('e1', 'Anna', 4500), dist('e2', 'Bob', 5500)],
        available: [avail('eur_50', 1, 5000), avail('eur_20', 2, 2000), avail('eur_5', 2, 500)],
      });
  ...
  describe('swap improvement phase', () => {
    it('improves fairness via swaps when greedy overshoots one employee', () => {
      ...
      const result = matchDenominations({
        distributions: [dist('e1', 'Anna', 3000), dist('e2', 'Bob', 2000)],
        available: [avail('eur_20', 2, 2000), avail('eur_10', 1, 1000)],
      });
    ...
    it('swaps denomination to new employee who had none of that type', () => {
      ...
      const result = matchDenominations({
        distributions: [dist('e1', 'Anna', 1000), dist('e2', 'Bob', 1000)],
        available: [avail('eur_10', 2, 1000)],
      });

      // Both should get €10 each
      expect(result.payouts[0]!.actualAmountInCents + result.payouts[1]!.actualAmountInCents).toBe(
        2000,
      );
    });
  ...
  describe('coins scenario', () => {
    it('handles exact change with small denominations', () => {
      const result = matchDenominations({
        distributions: [dist('e1', 'Anna', 350), dist('e2', 'Bob', 150)],
        available: [avail('eur_2', 2, 200), avail('eur_1', 1, 100), avail('eur_50ct', 2, 50)],
      });
    ...
    it('handles mixed bills and coins', () => {
      const result = matchDenominations({
        ...
        available: [avail('eur_10', 1, 1000), avail('eur_2', 1, 200), avail('eur_50ct', 1, 50)],
      });
