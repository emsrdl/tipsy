...
describe('useSmartSplitter', () => {
  it('returns null output when no employees', () => {
    const { result } = renderHook(() =>
      useSmartSplitter([], 10000, 0, denominations)
    )
  ...
  it('returns null output when zero total', () => {
    const { result } = renderHook(() =>
      useSmartSplitter(employees, 0, 0, denominations)
    )
    expect(result.current.output).toBeNull()
  })
  ...
  it('computes output when inputs are valid', () => {
    const { result } = renderHook(() =>
      useSmartSplitter(employees, 10000, 0, denominations)
    )
  ...
  it('respects kitchen percent', () => {
    ...
    const { result } = renderHook(() =>
      useSmartSplitter(mixedEmployees, 10000, 50, denominations)
    )
